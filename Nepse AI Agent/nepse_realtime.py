        ...
        async with pg_conn.transaction():
            ...
            for stock in data.get('COMPANY_DATA', []):
                try:
                    ...
                    current_price = float(stock.get('currentPrice', 0))
                    percentage_change = float(stock.get('percentageChange', 0))
                    volume = float(stock.get('volume', 0))
                    previous_closing = float(stock.get('previousClosing', 0))
                    open_price = float(stock.get('open', 0))
                    high_price = float(stock.get('high', 0))
                    low_price = float(stock.get('low', 0))
                    fifty_two_week_high = float(stock.get('fiftyTwoWeekHigh', 0))
                    fifty_two_week_low = float(stock.get('fiftyTwoWeekLow', 0))
                    market_capitalization = float(stock.get('marketCapitalization', 0))
                    pe_ratio = float(stock.get('peRatio', 0))
                    eps = float(stock.get('eps', 0))
                    book_value = float(stock.get('bookValue', 0))
                    dividend_yield = float(stock.get('dividendYield', 0))
                    week_change = float(stock.get('weekChange', 0))
                    day_change = float(stock.get('dayChange', 0))
...
async def get_latest_market_data() -> Dict:
    """Get the latest market data from Redis"""
    if not r:
...
async def get_historical_data(
    ...
    """Get historical stock data from PostgreSQL"""
    if not pg_conn:
...
async def get_market_research(background_tasks: BackgroundTasks) -> Dict:
    """Run general market research using the DeepResearchAgent"""
...
async def get_stock_research(symbol: str, background_tasks: BackgroundTasks) -> Dict:
    """Run deep research analysis for a specific stock"""
    if not research_agent:
...
async def get_all_trading_signals(
    ...
    try:
        ...
        for symbol in symbols:
            try:
                # Fetch recent historical data (e.g., last 30 days) for analysis
                # Adjust query as per your data granularity and agent requirements
                historical_data = await pg_conn.fetch(
                    "SELECT * FROM stock_data WHERE symbol = $1 ORDER BY created_at DESC LIMIT 30",
                    symbol
                )
                if not historical_data:
                    logger.warning(f"No historical data for {symbol}, skipping trading signal generation.")
                    continue
                
                # Convert records to dicts for the agent
                formatted_data = [dict(r) for r in historical_data]

                # Run trading signal generation for each stock
                signal = await trading_agent.generate_trading_signal(formatted_data, force_refresh)
